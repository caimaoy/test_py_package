"""
 ...
 This is the util module which provides common functions.
...
class CommonFile(object):
    """CommonFile object which is used to wrap the file and caculate md5 etc. """
    def __init__(self, file_path):
        """CommonFile init method. """
...
def md5sum_file(one_file):
    """md5sum of one file. """
    if not isinstance(one_file, CommonFile):
        error_exit("not a common type of file to caculate md5")
    m = hashlib.md5()
    f = one_file.get_file_object()
    if not f:
...
def relative_path(a_path, reference_path):
    ...
    for i in range(min(len(reference_list), len(path_list))):
        # TODO(chong): Why use lower here?
        if reference_list[i].lower() != path_list[i].lower():
            break
        else:
            # TODO(chong): Why do not move i+=1 out from the loop?
            i += 1
